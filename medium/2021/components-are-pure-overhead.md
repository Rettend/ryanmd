...
lastmod: 2021-05-09
source: https://ryansolid.medium.com/components-are-pure-overhead-12358123bc2b
---
...
## Components are something we take for granted in UI Frameworks, but should we?

[![](https://miro.medium.com/v2/resize:fill:32:32/1*eMR1FyWRupD7Ex07HZykGA.jpeg)](/?source=post_page---byline--12358123bc2b---------------------------------------)[Ryan Carniato](/?source=post_page---byline--12358123bc2b---------------------------------------)Follow6 min read·May 10, 2021[](https://medium.com/m/signin?actionUrl=https%3A%2F%2Fmedium.com%2F_%2Fvote%2Fp%2F12358123bc2b&operation=register&redirect=https%3A%2F%2Fryansolid.medium.com%2Fcomponents-are-pure-overhead-12358123bc2b&user=Ryan+Carniato&userId=1789bbc16f7b&source=---header_actions--12358123bc2b---------------------clap_footer------------------)283

7

[](https://medium.com/m/signin?actionUrl=https%3A%2F%2Fmedium.com%2F_%2Fbookmark%2Fp%2F12358123bc2b&operation=register&redirect=https%3A%2F%2Fryansolid.medium.com%2Fcomponents-are-pure-overhead-12358123bc2b&source=---header_actions--12358123bc2b---------------------bookmark_footer------------------)Listen

Share

Press enter or click to view image in full size![](https://miro.medium.com/v2/resize:fit:700/1*OejrIykrCKOC5kenfRDmcg.jpeg)
...
Press enter or click to view image in full size![](https://miro.medium.com/v2/resize:fit:700/1*s3JmD5Ibudz-DEE2jpSbBw.png)
...
SubscribeSubscribeWhile we were busy focusing on how VDOM libraries do all this potentially unnecessary work, we weren’t paying attention to our reactive libraries doing all this unnecessary memoization.
...
export function Chart({ data, enabled, headerText }) {  const el = useRef();  useEffect(() => {    let chart;    if (enabled) chart = new Chart(el.current, data);    return () => chart?.release();  }, [enabled]);  return (    <>      <h1>{headerText}</h1>      {enabled && <div ref={el} />}    </>  );}How many different places are we doing additional checks due to props.enabled? Can you find all 4? This isn't React specific. Equivalent code in Svelte touches 3 locations.
...
function Toggle() {  const [on, setOn] = useState(false)  const toggle = () => setOn(o => !o)  return <Switch on={on} onToggle={toggle} />}function Switch({on, onToggle}) {  return (    <div>      <SwitchMessage on={on} />      <SwitchButton onToggle={onToggle} />    </div>  )}function SwitchMessage({on}) {  return <div>The button is {on ? 'on' : 'off'}</div>}function SwitchButton({onToggle}) {  return <button onClick={onToggle}>Toggle</button>}## Vanishing Components

![](https://miro.medium.com/v2/resize:fit:600/1*G9dt10s3QuS3PsYhDnwJ9A.jpeg)
...
Originally published at [https://dev.to](https://dev.to/this-is-learning/components-are-pure-overhead-hpm) on May 10, 2021.